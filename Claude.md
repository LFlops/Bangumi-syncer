...
- **文档路径 (统一归档)**：
  - 所有文档均统一存放在 `specs/` 目录下，严禁创建 `docs/` 目录。
  - **目录命名规范**：必须以当前**任务 (Task)** 或**计划 (Plan)** 命名子目录。
    - ❌ 错误：`specs/bangumi-syncer/auth.md` (不要以项目名命名)
    - ✅ 正确：`specs/refactor-auth-logic/spec.md` (以任务命名)
    - ✅ 正确：`specs/feat-add-notify/plan.md`
- **执行闭环**：
  1.  **需求摄入**：接收功能请求或 Bug 报告。
  2.  **更新规格**：在 `specs/{task_name}/` 下新建或更新文档。
  3.  **制定计划**：基于新规格输出分步实施计划。
...
- **肥尾风险控制 (上下文重置)**：
  ...
  - 重置前务必将关键决策摘要追加至 `specs/decisions.md` (全局决策记录)。
...
- **文档优先**：任何变更必须同步更新 `specs/{task_name}/` 下的相关文档。
