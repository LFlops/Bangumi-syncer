...
| 位置 | 展示内容 | 数据来源 |
|------|---------|---------|
| Dashboard | 新增 LLM 用量卡片组（本月调用次数/Token/平均延迟，仿现有 stat cards 模式），放在现有 4 张卡片旁边 | `GET /llm/stats?scope=aggregate` |
| config.html | AI 追番总结区的 Job 卡片含该 job 用量小计 | `GET /llm/stats` (含 by_job 分组) |
...
- 数据获取：`loadDashboardData()` 中新增 `GET /llm/stats?scope=aggregate` 调用
...
|--------|------|-------------|
| GET | `/llm` | 获取 LLM 配置（api_key 脱敏） |
| PUT | `/llm` | 更新 LLM 配置 |
| POST | `/llm/test` | 测试 LLM 连接 |
| GET | `/llm/stats` | LLM 用量统计（聚合数据 + 按 model/job 分组 + 每日趋势） |
