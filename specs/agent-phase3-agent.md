...
记忆检索的语义化演进方向（Phase 3 期间按需评估，不在 Phase 3 首版范围）：

### 可 RAG 的候选（按价值排序）

| 候选 | 场景 | 词汇鸿沟 | 说明 |
|---|---|---|---|
| **bangumi_archive（2GB 离线档案）** | 标题匹配（用户输入 → 番剧条目） | **高**（中文/日文/别名/英文） | **项目内最合适**——`_fts_query.py` 现状是 FTS5 标题搜索，跨语言/别名是真实失败点 |
| knowledge_base（错误模式沉淀） | 诊断"类似错误" | 中（措辞多样） | 语义检索"服务器挂了" ↔ "503 错误" |
| 记忆归档（search_archive） | 失败模式全量历史 | 中 | 规模大（数万条）后再上 |

**2GB 档案的可行性**：一次构建（embedding 全量条目）+ 增量更新；向量存储用 SQLite + 向量列（numpy 计算或 sqlite-vec 扩展）——与现有 SQLite 基础设施一致。

### 召回（retrieval）：混合检索

- **FTS5 关键词 + 向量检索并集**（BM25 词法 + 语义互补）——提高召回率
- 查询词来源不变（调用方提供，见 Phase 2 关键词来源）
- 实施原则：**FTS5 先精确匹配，RAG 兜底语义**（增强而非替换）

### 重排序（rerank）：RRF 起步 → LLM 精排

- **RRF（Reciprocal Rank Fusion）**：融合 FTS5 排名与向量排名——零模型依赖，起步推荐
- **LLM 精排**：Phase 3 已有 LLM——对 top-K 候选打分/选择，灵活但慢（按需启用）
- cross-encoder rerank 模型（bge-reranker）：精度最高但引入模型依赖（暂不评估）

## 前端入口：日志页面独立按钮

**入口固定在日志（logs）页面**——历史异常模式识别是独立功能，与观看记录总结（summary）解耦：

```
日志页面 → 错误日志列表顶部 "AI 分析" 按钮
    → 触发 POST /api/agent/run（task: "分析最近 N 小时错误日志"）
    → 轮询 GET /api/agent/runs/{run_id} 追踪执行
    → 展示：思考步骤 + 工具调用 + 诊断报告 + 知识沉淀确认（commit-knowledge）
```

- **为什么放日志页**：用户看到错误日志的上下文里触发分析，与"追番总结"功能内聚性分离
- **实现顺序**：API 层（`/api/agent/run`）先行可用（curl/Swagger 验证），日志页按钮在 Agent 核心跑通后补
