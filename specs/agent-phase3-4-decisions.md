...
#### D4. 结构化输出
- **问题**：总结为自由文本，full_text 无结构，后续通知渲染与解析都受限（E2 定稿：FTS 检索的召回改善由摘要约束解决，见 `agent-phase2-closeout.md` §4）。
...
#### A1/A2. Phase 编号重定义与 feedback 排期 ✅ 已裁决（详见 `agent-phase2-closeout.md` §1）

- **Phase 2 定稿为记忆三件套（2.0.1–2.0.3，已完成）**；feedback → Phase 3.1；工具协议（原 2.1）→ Phase 4.x；openai 重构（原 2.2）已实施。
- Phase 2 的审查清单（W 系列）、决断（E 系列）、双 provider 异同对照、手工验收指南：**已整体迁出至 `specs/agent-phase2-closeout.md`**（本文档不再承载 Phase 2 内容）。
...
#### B2. D3 关键词提取时机与成本说法矛盾 ✅ 提案取消（被联表反查吸收）
- 历史：keywords 用于生成前的记忆检索，曾讨论 LLM 提取关键词的时机/成本矛盾。
- **取消结论（2026-08）**：相关回忆改由 `get_related_titles` 联表反查承担（`consumed_run_id` 显式关联，输入即今日 bgm_title 去重，无需 LLM 提取）；`search_fts`/keywords 通路保留 deprecated 供 Phase 5 混合检索评估。详见 `agent-phase2-closeout.md` §4 E2。
